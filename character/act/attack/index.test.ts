...
import useAttack from './index.ts'
import { createDefaultOwner, defaultCharacterSheet } from '../../../defaults/index.ts'
...
import { toPlainModLogs } from '../../../stat-modifier/log/format.ts'
...
describe('Visualize useAttack logs:', () => {
    test('create the file', () => {
        const result = useAttack(createDefaultOwner({
            es: { mainhand: shortsword },
...
describe('useAttack works with attacks', () => {
...
describe('simulation: useAttack works with feats', () => {
    ...
    test('equipment damage can touch the character sheet', () => {
        const data = createDefaultOwner({
            cs: { ...defaultCharacterSheet, dex: 10, str: 16 },
            es: { mainhand: strDagger },
        })
...
describe('useAttack works with crits', () => {
    ...
    test('Both feats and equipment can increase threat range', () => {
        const result = useAttack(createDefaultOwner({
            es: { mainhand: shortsword, amulet: critAmulet },
            fs: { featImprovedCritical },
        }))
